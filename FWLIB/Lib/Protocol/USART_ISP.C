...
*返回值			:	无
*******************************************************************************/
void BSP_Conf(BSP_Conf_TypeDef *BSP_Conf)		//配置函数
{
...
void USM_BSP_PROCESS(BSP_Conf_TypeDef *BSP_Conf)
{
	...
	USM_BSP_EraseMemory(BSP_Conf);			//擦除一个到全部 Flash 页面	
...
void USM_BSP_EraseMemory(BSP_Conf_TypeDef *BSP_Conf)		//擦除一个到全部 Flash 页面
{
	...
	{
		if(BSP_Conf->BSPM_Info.ACK==BSP_ACK)		//等待应答
		{	
			...
			{
				USM_BSP_SetStatus(BSP_Conf,BSP_MSTATUS_WriteWM);		//更新状态---等待释放从机
				BSP_Conf->BSP_DATA.TimeCount=0;			//清除计数
				BSP_Conf->BSP_DATA.RetryTimes=0;		//重试计数清零
				BSP_Conf->BSPM_Info.ACK=BSP_NACK;		//清除应答标志
