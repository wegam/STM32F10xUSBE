...
* 返回值			: void
*******************************************************************************/
void CAN_Configuration_NR(u32 CAN_BaudRate)					//CAN配置--标志位查询方式
{
//	CAN_INIT();
	//基本配置：以系统时钟72MHz为计算，RCC_APB1PeriphClock时钟频率为36MHz,配置最高波特率为4MHz
	//根据传入参数(CAN_BaudRate)自动计算分频值以得到实际需要波特率
	//配置过程：
	//1)定义相关结构体
	GPIO_InitTypeDef	GPIO_InitStructure;					//GPIO结构体
	CAN_InitTypeDef		CAN_InitStructure;					//CAN结构体
	
	u16 CAN_Prescaler=0;	//CAN_Prescaler设定了一个时间单位的长度，它的范围是1到1024。
	...
	//3)************GPIO脚配置，使用CAN1默认引脚，其它引脚后续再补充	
	//3.1)**********CAN_RX默认接收引脚配置，PA11
	GPIO_InitStructure.GPIO_Pin		= GPIO_Pin_11; 
	GPIO_InitStructure.GPIO_Speed	= GPIO_Speed_50MHz;
	GPIO_InitStructure.GPIO_Mode	= GPIO_Mode_IPU;
	GPIO_Init(GPIOA, &GPIO_InitStructure);
	//3.2)**********CAN_TX默认发送引脚配置，PA12
	...
	//将RCC_APB1PeriphClock/(CAN_SJW+CAN_BS1+CAN_BS2)设置为4MHz
	//CAN_Prescaler根据需要的波特率自动计算分频值
	//注意：总体配置保持tBS1>=tBS2 tBS2>=1个CAN时钟周期  tBS2>=2tSJW
  ...
  CAN_InitStructure.CAN_Mode=CAN_Mode_Normal;			//CAN_Mode设置了CAN的工作模式，<CAN operating mode>.给出了该参数可取的值
	...
	//4.1)**********CAN波特率配置
	//4.1.1)********根据需求波特率设置分频值
	CAN_Prescaler=4000000/CAN_BaudRate;
...
//		warning: "sdfdf";
  ...
  CAN_InitStructure.CAN_BS1=CAN_BS1_5tq;	//CAN_BS1设定了时间段1的时间单位数目，<time quantum in bit segment 1>给出了该参数可取的值
	
  CAN_InitStructure.CAN_BS2=CAN_BS2_3tq;	//CAN_BS2设定了时间段1的时间单位数目，<time quantum in bit segment 2>给出了该参数可取的值
	
  CAN_InitStructure.CAN_Prescaler=CAN_Prescaler;			//CAN_Prescaler设定了一个时间单位的长度，它的范围是1到1024。
...
* 返回值			: void
*******************************************************************************/
void CAN_Configuration_IT(u32 CAN_BaudRate)					//CAN配置---中断方式
{
//	CAN_INIT();
	//基本配置：以系统时钟72MHz为计算，RCC_APB1PeriphClock时钟频率为36MHz,配置最高波特率为4MHz
	//根据传入参数(CAN_BaudRate)自动计算分频值以得到实际需要波特率
...
{
	//说明：
	//--在CAN协议里，报文的标识符不代表节点的地址，而是跟报文的内容相关的。因此，发送者以广播的形式把报文发送给所有的接收者。
	//节点在接收报文时,根据标识符(CAN ID)的值决定软件是否需要该报文；如果需要，就拷贝到SRAM里；如果不需要，报文就被丢弃且无需软件的干预。
  //--为满足这一需求，bxCAN为应用程序提供了14个位宽可变的、可配置的过滤器组(13~0)，以便只接收那些软件需要的报文。硬件过滤的做法节省了CPU开销，
	//否则就必须由软件过滤从而占用一定的CPU开销。每个过滤器组x由2个32位寄存器，CAN_FxR0和CAN_FxR1组成。
	...
	//bxCAN模块可以完全自动地接收和发送CAN报文；且完全支持标准标识符STID(11位000-7FF)和扩展标识符EXID(29位0000 0000-1FFF FFFF)。
...
//	CAN_ID=0X317;
  ...
  CAN_FilterInitStructure.CAN_FilterNumber=CAN_GROUP;						//设置过滤器组0，范围为0~13
	...
	//1.3)**********下面根据设置的参数不同来决定can总线can总线的配置情况：注：slave_id为要过滤的id号。
		/*
			//1、对扩展数据帧进行过滤:(只接收扩展数据帧)	(注：标准帧数据帧、标准远程帧和扩展远程帧均被过滤)
  ...
  CAN_FilterInitStructure.CAN_FilterMode=CAN_FilterMode_IdMask;	//STM32提供两种过滤模式供用户设置：屏蔽位模式和标识符列表模式。
  ...
  CAN_FilterInitStructure.CAN_FilterScale=CAN_FilterScale_32bit;//过滤器位宽
	...
	//1.3.4---------标识位寄存器的设置 (发送邮箱标识符寄存器 (CAN_TIxR) (x=0..2))																															
  CAN_FilterInitStructure.CAN_FilterIdHigh=(((u32)CAN_ID<<21)&0xffff0000)>>16;	//设定过滤器标识符（32位位宽时为其高段位，16位位宽时为第一个）。它的范围是0x0000到0xFFFF。																																
  CAN_FilterInitStructure.CAN_FilterIdLow=0x0000;																//设定过滤器标识符（32位位宽时为其低段位，16位位宽时为第二个）。它的范围是0x0000到0xFFFF。
	//1.3.5---------屏蔽寄存器的设置 
  CAN_FilterInitStructure.CAN_FilterMaskIdHigh=0xFFFF;					//设定过滤器屏蔽标识符或者过滤器标识符（32位位宽时为其高段位，16位位宽时为第一个）。它的范围是0x0000到0xFFFF。
  CAN_FilterInitStructure.CAN_FilterMaskIdLow=0xFFFF;						//设定过滤器屏蔽标识符或者过滤器标识符（32位位宽时为其低段位，16位位宽时为第二个）。它的范围是0x0000到0xFFFF。
	//1.3.6---------过滤器组关联
...
* 返回			: 无
*******************************************************************************/
void CAN_FilterInitConfiguration_ExtData(
																	u8 CAN_GROUP,			//过滤器组---bxCAN为应用程序提供了14个位宽可变的、可配置的过滤器组(13~0)，以便只接收那些软件需要的报文
																	u32 CAN_ID,				//
																	u32 MaskId				//
)		//CAN滤波器配置
...
//	CAN_ID=0X317;
  ...
  CAN_FilterInitStructure.CAN_FilterScale=CAN_FilterScale_32bit;//过滤器位宽
																																...
																																//->CAN_FilterScale_One32bit：1个32位过滤器
	//1.3.4---------标识位寄存器的设置 (发送邮箱标识符寄存器 (CAN_TIxR) (x=0..2))																															
  CAN_FilterInitStructure.CAN_FilterIdHigh=(((u32)CAN_ID<<3)&0xFFFF0000)>>16;;						//设定过滤器标识符（32位位宽时为其高段位，16位位宽时为第一个）。它的范围是0x0000到0xFFFF。																																
  CAN_FilterInitStructure.CAN_FilterIdLow=(((u32)CAN_ID<<3)|CAN_ID_EXT|CAN_RTR_DATA)&0xFFFF;;								//设定过滤器标识符（32位位宽时为其低段位，16位位宽时为第二个）。它的范围是0x0000到0xFFFF。
	//1.3.5---------屏蔽寄存器的设置 
  CAN_FilterInitStructure.CAN_FilterMaskIdHigh=0xFFFF;					//设定过滤器屏蔽标识符或者过滤器标识符（32位位宽时为其高段位，16位位宽时为第一个）。它的范围是0x0000到0xFFFF。
  CAN_FilterInitStructure.CAN_FilterMaskIdLow=0xFFFF;						//设定过滤器屏蔽标识符或者过滤器标识符（32位位宽时为其低段位，16位位宽时为第二个）。它的范围是0x0000到0xFFFF。
	//1.3.6---------过滤器组关联
...
void CAN_StdTX_DATA(
										u16 CAN_ID,			//CAN地址
										u8 length,				//设定待传输消息的帧长度。它的取值范围是0到8。
										u8 *Data				//数据										
)
{
	CanTxMsg TxMessage;
...
////******************CAN波特率100K		//总体配置保持tBS1>=tBS2 tBS2>=1个CAN时钟周期  tBS2>=2tSJW
//	
//	CAN_InitStructure.CAN_SJW=CAN_SJW_1tq;	//CAN_SJW定义了重新同步跳跃宽度(SJW)，即在每位中可以延长或缩短多少个时间单位的上限，<CAN synchronisation jump width>给出了该参数可取的值
//																					//CAN_SJW_1tq:重新同步跳跃宽度1个时间单位
//																					//CAN_SJW_2tq:重新同步跳跃宽度2个时间单位
...
//  CAN_InitStructure.CAN_BS2=CAN_BS2_3tq;	//CAN_BS2设定了时间段1的时间单位数目，<time quantum in bit segment 2>给出了该参数可取的值
//	
//  CAN_InitStructure.CAN_Prescaler=40;			//CAN_Prescaler设定了一个时间单位的长度，它的范围是1到1024。
//	
//  CAN_Init(&CAN_InitStructure);
//	
