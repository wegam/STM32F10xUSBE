...
//	PWM_OUT2(TIM1,PWM_OUTChannel1,2400,600);		//PWM设定
	PWM_OUT2(TIM3,PWM_OUTChannel3,1200,PWM_Ratio);		//PWM设定
	PWM_OUT2(TIM3,PWM_OUTChannel4,1200,PWM_Ratio);		//PWM设定
//	PWM_OUT2(TIM4,PWM_OUTChannel2,2400,800);		//PWM设定
...
//			TIM3->CCR4 =PWM_Ratio*STep;
		...
		else if(RevBuffer[0]==0x53&&RevBuffer[1]==0x45&&RevBuffer[2]==0x54&&RevBuffer[3]==0x3A)
		{
			...
			TIM3->CCR3 =PWM_Ratio;
			TIM3->CCR4 =PWM_Ratio;
			Npow=(double)PWM_Ratio/STep;
//			PWM_Ratio	=(u16)Npow;
			USART_DMAPrintf(USART1,"当前输出比:%5d/60000；\t等级比:%4.2f/1200；\t 占空比：%4.3f%%\r\n",PWM_Ratio,Npow,Npow/1200*100);					//自定义printf串口DMA发送程序,后边的省略号就是可变参数
		}
...
void PWM_ECODE2(void)
{
	if(PB12in==0&&ECountA==0&&ECountB==0)			//EXTI_GetFlagStatus(u32 EXTI_Line);//PA6in==0
	{
		if(Key1Flag==0)
		{
			...
			if(KeyCount>=500)
			{
				...
				USART_DMAPrintf(USART1,"当前输出比:%5d/60000；\t等级比:%4.2f/1200；\t 占空比：%4.3f%%\r\n",PWM_Ratio,(double)PWM_Ratio/STep,(PWM_Ratio/60000.0)*100);					//自定义printf串口DMA发送程序,后边的省略号就是可变参数	
				TIM3->CCR3 =PWM_Ratio;
				TIM3->CCR4 =PWM_Ratio;				
			}
		}
	...
	if(PB13in==0&&ECountA<0xFF&&ECountB<0xFF&&KeyCount==0)			//EXTI_GetFlagStatus(u32 EXTI_Line);//PA6in==0
	{
		ECountA++;
	}
	if(PB14in==0&&ECountA<0xFF&&ECountB<0xFF&&KeyCount==0)
	{
		ECountB++;
	}
	if((PB13in&&PB14in)&&(ECountB>10||ECountA>10))
	{
		if(ECountA>ECountB)
		{
			if(PWM_Ratio<60000)
